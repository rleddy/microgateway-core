...
module.exports.init = function init(stubConfig) {
  ...
  const writeLogRecordToFile = function (record,cb) {
    if (Date.now() > nextRotation) {
     ...
     if (logFileFd) {
      ...
      if (cluster.isMaster) {
        if(!logToConsole) {
          writeConsoleLog('log','logging to ' + logFilePath);
        }
      }
    ...
    fs.write(logFileFd, buffer, offset, 'utf8', function (err, written) {
      ...
      if (err) {
        if (!logFailWarn) {
          ...
          writeConsoleLog('warn','error writing log',err);
          logFailWarn = true;
        }
      } else {
        offset += written;
  ...
  logger = {
    ...
    consoleLog: function (level, ...data) {
      return writeConsoleLog(level, ...data);
    },
  ...
  if (cluster.isMaster) {
    if(logToConsole) {
      writeConsoleLog('log','logging to console');
    } 
...
>>>>>>> 93bdd8a... Creating centralized console logging
      }
    } else if (key === 'res') {
      const res = obj[key];
      if (res) {
